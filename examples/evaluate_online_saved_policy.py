            ...
            while (
                ...
                with torch.no_grad():
                    if self._actor is not None:
                        ...
                        # print(self._actor_critic.cost_critic.forward(obs,act))
                    elif self._planner is not None:
                        act = self._planner.output_action(
                            obs.unsqueeze(0).to('cpu'),
                        )[
...
if __name__ == '__main__':
    ...
    file_names = [item.name for item in scan_dir if item.is_file()]
    file_names.sort(key=extract_number)
    lastest_file = file_names[-1] if file_names else None

    # if lastest_file.is_file() and lastest_file.name.split('.')[-1] == 'pt':
    evaluator.load_saved(
            save_dir=LOG_DIR,
            model_name=lastest_file,
            camera_id=1,
            width=256,
            height=256,
    )
